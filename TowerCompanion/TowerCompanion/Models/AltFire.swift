...
struct AltFire: Codable, Hashable {
    ...
    static func getAltFireDescription(for altfireName: String) -> String {
        let allAltfires = getAllAltFires()
        return allAltfires.first(where: { $0.name == altfireName })?.altFireDescription ?? "AltFire Description Can't Be Found."
    }
